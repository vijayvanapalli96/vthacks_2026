...
createServer(async (request, response) => {
  ...
  if (request.method === "POST" && request.url === "/a2a/apply") {
    ...
    try {
      ...
      if (!packet?.applicant_agent) {
        return send(response, 403, { status: "refused", reason: "A claimed applicant ANS identity is required." });
      }
      const verification = await verifyRemoteAgent({ ansName: packet.applicant_agent, expectedRole: "applicant" });

...
export async function loadEmployerEvidence(agentId = EMPLOYER_AGENT_ID): Promise<{
  evidence: EmployerEvidence;
  registry: { agent_id: string; ans_name: string; status: string; metadata_reachable: boolean };
}> {
  const query = encodeURIComponent(EMPLOYER_ANS_NAME);
  const [registryPayload, transparencyPayload] = await Promise.all([
    fetchJson(`${registryUrl}?query=${query}`),
    fetchJson(`${transparencyUrl}/${encodeURIComponent(agentId)}`),
  ]);

  const items = (registryPayload as { items?: RegistryAgent[] }).items ?? [];
  const agent = items.find((item) => item.agentId === agentId);
  if (!agent) throw new Error('The employer agent was not found in the public ANS registry.');

  const event = (transparencyPayload as {
...
export async function verifyProductionEmployer(agentId = EMPLOYER_AGENT_ID) {
  const { evidence, registry } = await loadEmployerEvidence(agentId);

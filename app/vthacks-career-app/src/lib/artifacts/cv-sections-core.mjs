// cv-sections-core.mjs — LIFTED VERBATIM from the career-ops donor repo.
//
// ─────────────────────────────────────────────────────────────────────────────
// Source:  career-ops/cv-sections-core.mjs  (v1.33.0)
// License: MIT — Copyright (c) 2026 Santiago Fernández de Valderrama
//          <hi@santifer.io> · https://santifer.io
// career-ops is a DONOR REPO, not a dependency. Copied byte-for-byte (`cp`),
// never imported across the repo boundary; `../career-ops/` is never edited.
// ─────────────────────────────────────────────────────────────────────────────
//
// NOTHING BELOW THIS HEADER IS CHANGED. The file has ZERO imports, which is
// why it is the one file in the whole lift that is a literal copy: there was
// nothing to unwire. Every character from `// Shared optional-section
// stripping` down is the donor's.
//
// HIREWIRE uses it from `render-html.mjs`, whose inline print template honours
// the marker contract this file documents at length — `<!-- WORK EXPERIENCE -->`
// style all-caps markers, and the `<!-- END -->` sentinel after the last
// section. Read "The Skills sentinel" note below BEFORE editing that template:
// a template that ends on Skills with no sentinel fails safe (bare header), but
// only because these patterns deliberately lack an `|$` branch. Do not add one.
//
// Why it earns its place: a student with no listed projects should not get a
// resume with an empty "Projects" heading on it, and a student with no degree
// row should not get an empty "Education" heading. Both happen, because the
// template wraps all eight sections unconditionally.

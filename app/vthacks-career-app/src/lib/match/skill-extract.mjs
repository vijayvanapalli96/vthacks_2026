/**
 * LIFTED VERBATIM from career-ops v1.33.0 `skill-extract.mjs`.
 *
 *   Copyright (c) 2026 Santiago Fernández de Valderrama
 *   MIT License. https://github.com/santifer/career-ops
 *
 * Copied, not imported: career-ops is a sibling DONOR repo, not a dependency
 * (see CLAUDE.md). The only edits are this header and the removal of the
 * `#!/usr/bin/env node` shebang, which is meaningless for a module the Next
 * bundler imports. The vocabulary, the regexes, the CANONICAL map and every
 * design note below are byte-identical to the original — deliberately, because
 * the notes are the reason the choices are correct, and paraphrasing them would
 * lose the measurements that justify each omission.
 *
 * ─────────────────────────────────────────────────────────────────────────────
